...
class DeliveryCompleteOTPPanel extends StatefulWidget {
  final OrderModel? orderModel;
  final Function? resendDeliveryCompleteCodeCallback;
  final Function? deliveryCompleteCallback;

  DeliveryCompleteOTPPanel({
    Key? key,
    @required this.orderModel,
    @required this.resendDeliveryCompleteCodeCallback,
    @required this.deliveryCompleteCallback,
  }) : super(key: key);

  @override
  _DeliveryCompleteOTPPanelState createState() => _DeliveryCompleteOTPPanelState();
}

class _DeliveryCompleteOTPPanelState extends State<DeliveryCompleteOTPPanel> {
                ...
                children: [
                  GestureDetector(
                    onTap: () {
                      ...
                      widget.resendDeliveryCompleteCodeCallback!();
                    },
                    child: Container(
              ...
              if (deliveryOTPProvider.deliveryOTPState.progressState == 2)
                KeicyRaisedButton(
                  ...
                  onPressed: _otpController.text.length != 4
                      ? null
                      : () {
                          if (deliveryOTPProvider.deliveryOTPState.customerDeliveryCode != _otpController.text) {
                            OTPInvalidDialog.show(
                              context,
                              widthDp: widthDp,
                              heightDp: heightDp,
                              fontSp: fontSp,
                              text: "The Delivery OTP is incorrect. Please check with customer and enter or re-send OTP again.",
                              resendOTPCallback: () {
                                _otpController.clear();
                                widget.resendDeliveryCompleteCodeCallback!();
                              },
                            ...
                            widget.deliveryCompleteCallback!();
                          }
                        },
                ),
            ],
